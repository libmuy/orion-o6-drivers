...
static int rx8900_set_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	...
	if (tmp_alarm_value > 0 && tmp_alarm_value < 60)
...
static void rx8900_shutdown(struct i2c_client *client)
{
	...
	if (seconds_alarm_values) {
		...
		rx8900_read_regs(client, RX8900_BTC_TIMER_CNT_0, 2, counter);
...
static int rx8900_resume(struct device *dev)
{
	...
	flags &= ~(RX8900_BTC_FLAG_AF|RX8900_BTC_FLAG_TF|RX8900_BTC_FLAG_UF);
	// acknowledge IRQ
	rx8900_write_reg(client, RX8900_BTC_FLAG, 0x0f & flags);
...
static int rx8900_suspend(struct device *dev)
{
	...
	/* Config FIXED timer counter0, counter1*/
	err = rx8900_read_regs(client, RX8900_BTC_TIMER_CNT_0, 2, counter);
	if (err)
		return err;
	...
	err = rx8900_write_reg(rx8900->client, RX8900_BTC_CTRL, ctrlreg);
	if (err)
		return err;
