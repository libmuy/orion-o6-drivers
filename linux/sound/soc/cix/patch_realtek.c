...
static int snd_hda_dmic1_switch_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	int pinctls = snd_hda_codec_read(codec, 0x12, 0,
...
static int snd_hda_dmic1_switch_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	int is_switch = ucontrol->value.integer.value[0];
	...
	if (is_switch) {
		int dmic_pinctrl[] = {0x80, 0x01, 0xA6, 0x90};

		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_0, dmic_pinctrl[0]);
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_1, dmic_pinctrl[1]);
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_2, dmic_pinctrl[2]);
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_3, dmic_pinctrl[3]);
		...
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_0, 0x0);
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_1, 0x0);
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_2, 0x0);
		snd_hda_codec_write(codec, 0x12, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_3, 0x0);
...
static const struct snd_kcontrol_new dmic1_switch = {
	...
	.name = "Dmic1 Switch",
	.info = snd_hda_switch_info,
	.get = snd_hda_dmic1_switch_get,
	.put = snd_hda_dmic1_switch_put,
};

static int create_dmic1_switch(struct hda_codec *codec)
{
	struct snd_kcontrol *kctl;

	kctl = snd_ctl_new1(&dmic1_switch, codec);
	if (!kctl)
		return -ENOMEM;

	return snd_hda_ctl_add(codec, 0x12, kctl);
}

static int snd_hda_dmic2_switch_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	int pinctls = snd_hda_codec_read(codec, 0x13, 0,
					 AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
...
static int snd_hda_dmic2_switch_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	int is_switch = ucontrol->value.integer.value[0];
	...
	if (is_switch) {
		...
		snd_hda_codec_write(codec, 0x13, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_0, dmic_pinctrl[0]);
		snd_hda_codec_write(codec, 0x13, 0,
				    AC_VERB_SET_CONFIG_DEFAULT_BYTES_1, dmic_pinctrl[1]);
		snd_hda_codec_write(codec, 0x13, 0,
...
static const struct snd_kcontrol_new dmic2_switch = {
	...
	.name = "Dmic2 Switch",
	.info = snd_hda_switch_info,
	.get = snd_hda_dmic2_switch_get,
	.put = snd_hda_dmic2_switch_put,
};

static int create_dmic2_switch(struct hda_codec *codec)
{
	struct snd_kcontrol *kctl;

	kctl = snd_ctl_new1(&dmic2_switch, codec);
	if (!kctl)
		return -ENOMEM;

	return snd_hda_ctl_add(codec, 0x13, kctl);
}
...
int alc_build_controls(struct hda_codec *codec)
{
	...
	create_dmic1_switch(codec);
	create_dmic2_switch(codec);
...
const struct of_device_id snd_hda_id_realtek_of_match[] = {
	...
	{ .compatible = "realtek,alc3287", .data = &alc_codec_pdata},
	{}
};
