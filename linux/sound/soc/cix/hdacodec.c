	...
	for (i = 0; i < hdev->num_nodes; i++, nid++) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			if (type == AC_WID_AUD_IN) {
				unsigned int gain = snd_hdac_regmap_get_amp_stereo(hdev, nid,
										   HDA_INPUT, 0);

				if ((gain & 0xffff) & (0x1 << 7))
					snd_hdac_regmap_update_amp_stereo(hdev, nid, HDA_INPUT, 0,
									  0xffff,
									  (gain & 0xffff) & (~(0x1 << 7)));
