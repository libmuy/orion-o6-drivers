...
static int v4l2_try_start(AVCodecContext *avctx)
{
    ...
    if (capture->streamon)
        goto output_streamon;

    /* 1. get the capture format */
    ...
    /* 1.1 update the AVCodecContext */
    avctx->pix_fmt = ff_v4l2_format_v4l2_to_avfmt(capture->format.fmt.pix_mp.pixelformat, AV_CODEC_ID_RAWVIDEO);
    capture->av_pix_fmt = avctx->pix_fmt;

    /* 2. set the crop parameters */
    ...
    /* 3. init the capture context now that we have the capture format */
    if (!capture->buffers) {
...
static av_cold int v4l2_decode_init(AVCodecContext *avctx)
{
    ...
    if (ret) {
        av_log(avctx, AV_LOG_ERROR, "can't initialize mutex\n");
        return ret;
    }
...
static av_cold void v4l2_decode_flush(AVCodecContext *avctx)
{
    V4L2m2mContext *s = ((V4L2m2mPriv*)avctx->priv_data)->context;
    AVFrame *frame;
    V4L2Context *ctx;
    ...
    if (s->flushing || (!s->capture.streamon && !s->output.streamon))
        return;

    ret = ff_v4l2_context_set_status(&s->output, VIDIOC_STREAMOFF);
    if (ret) {
        av_log(s->avctx, AV_LOG_ERROR, "VIDIOC_STREAMOFF, on output context\n");
        goto exit;
    }

    status = av_mallocz(s->capture.num_buffers * sizeof(*status));
    if (!status)
        goto exit;
    ...
    if (s->capture.buffers) {
        for (i = 0; i < s->capture.num_buffers; i++)
            status[i] = s->capture.buffers[i].status;
    }

    ret = ff_v4l2_context_set_status(&s->capture, VIDIOC_STREAMOFF);
    if (ret) {
        pthread_mutex_unlock(&s->lock);
        av_log(s->avctx, AV_LOG_ERROR, "VIDIOC_STREAMOFF, on capture context\n");
        goto exit;
    }
    ...
    if (!frame) {
        av_log(s->avctx, AV_LOG_ERROR, "Failed to allocate av frame\n");
        goto exit;
    }

exit:
    if (!s->capture.streamon) {
        ret = ff_v4l2_context_set_status(&s->capture, VIDIOC_STREAMON);
        if (ret)
            av_log(s->avctx, AV_LOG_ERROR, "VIDIOC_STREAMON, on output context\n");

        if (s->capture.buffers && status) {
            ctx = &s->capture;
            for (i = 0; i < ctx->num_buffers; i++) {
                avbuf = &ctx->buffers[i];
                if (status[i] == V4L2BUF_AVAILABLE)
                    continue;
                avbuf->status = V4L2BUF_AVAILABLE;
                if (!ff_v4l2_buffer_buf_to_avframe(frame, avbuf)) {
                    for (int p = 0; p < avbuf->num_planes; p++)
                        av_buffer_unref(&frame->buf[p]);
                }
            }
        }
    }
    ...
    if (!s->output.streamon) {
        ...
        if (s->output.buffers) {
            for (i = 0; i < s->output.num_buffers; i++)
                s->output.buffers[i].status = V4L2BUF_AVAILABLE;
        }
    }
...
static const AVOption options[] = {
    ...
    { "num_capture_buffers", "Number of buffers in the capture context",
        OFFSET(num_capture_buffers), AV_OPT_TYPE_INT, {.i64 = V4L2_DEFAULT_CAPTURE_BUFFERS}, V4L2_DEFAULT_CAPTURE_BUFFERS, INT_MAX, FLAGS },
    { NULL},
};
