...
static const struct fmt_conversion {
    ...
    { AV_FMT(P010LE),      AV_CODEC(RAWVIDEO),    V4L2_FMT(P010M) },
    { AV_FMT(P010BE),      AV_CODEC(RAWVIDEO),    V4L2_FMT(P010M) },
...
static const struct fmt_v4l2_compatible_avfmt {
    enum AVPixelFormat avfmt;
    enum AVPixelFormat v4l2_avfmt;
} compatible_avfmt_map[] = {
    { AV_FMT(YUV420P10LE), AV_FMT(P010LE) },
    { AV_FMT(YUV420P10LE), AV_FMT(P010BE) },
};
...
enum AVPixelFormat ff_v4l2_format_avfmt_adjust(enum AVPixelFormat avfmt)
{
    int i;
    for (i = 0; i < FF_ARRAY_ELEMS(compatible_avfmt_map); i++) {
        if (compatible_avfmt_map[i].avfmt  == avfmt)
            return compatible_avfmt_map[i].v4l2_avfmt;
    }
    return AV_PIX_FMT_NONE;
}
