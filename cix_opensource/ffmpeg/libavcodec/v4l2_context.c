...
static inline unsigned int v4l2_get_pixelformat(struct v4l2_format *fmt)
{
...
static inline unsigned int v4l2_resolution_changed(V4L2Context *ctx, struct v4l2_format *fmt2, int min_buffers)
{
    ...
    if (min_buffers && ctx->num_buffers < min_buffers + V4L2_EXTRA_BUFFERING) {
        ret = 1;
        ctx->num_buffers = min_buffers + V4L2_EXTRA_BUFFERING;
    }

    if (ret)
        av_log(logger(ctx), AV_LOG_DEBUG, "%s changed (%s %dx%d) -> (%s %dx%d)\n",
            ctx->name,
            av_fourcc2str(v4l2_get_pixelformat(fmt1)),
            v4l2_get_width(fmt1), v4l2_get_height(fmt1),
            av_fourcc2str(v4l2_get_pixelformat(fmt2)),
...
static int v4l2_handle_event(V4L2Context *ctx)
{
    ...
    v4l2_get_ctrl(s->fd, V4L2_CID_MIN_BUFFERS_FOR_CAPTURE, &min_buffers);

    if (v4l2_resolution_changed(&s->capture, &cap_fmt, min_buffers)) {
        s->capture.av_pix_fmt = ff_v4l2_format_v4l2_to_avfmt(v4l2_get_pixelformat(&cap_fmt), AV_CODEC_ID_RAWVIDEO);
...
static inline int v4l2_try_raw_format(V4L2Context* ctx, enum AVPixelFormat *pixfmt)
{
    ...
    if (!v4l2_fmt) {
        /* Try to find v4l2 compatible format */
        enum AVPixelFormat v4l2_avfmt = ff_v4l2_format_avfmt_adjust(*pixfmt);
        if (v4l2_avfmt == AV_PIX_FMT_NONE) {
            return AVERROR(EINVAL);
        } else {
            *pixfmt = v4l2_avfmt;
            v4l2_fmt = ff_v4l2_format_avfmt_to_v4l2(*pixfmt);
